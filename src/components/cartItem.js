...
const CartItem = (props) => {
  ...
  return (
    ...
    <div className="flex items-center gap-3 py-3 border-b mx-auto max-w-4xl px-4">
     ...
     <div className="flex justify-between items-center w-full mt-1">
      <div className="flex items-center gap-2">
       <button className="bg-gray-300 rounded-full w-6 h-6 text-[#032b63] font-bold" onClick={handleMinusQuantity} > - </button>
       <span className="text-lg font-semibold">{quantity}</span>
       <button className="bg-gray-300 rounded-full w-6 h-6 text-[#032b63] font-bold" onClick={handlePlusQuantity} > + </button>
      </div>
    ...
    <Modal show={showRemoveModal} onHide={cancelRemove} centered>
     ...
     <Modal.Body> Are you sure you want to remove this item from the cart? </Modal.Body>
     <Modal.Footer>
      <Button variant="secondary" onClick={cancelRemove}> Cancel </Button>
