...
const Filter = () => {
  ...
  <div className="flex justify-between items-center p-5 border-b">
   ...
   <button onClick={handleCloseFilter} className="text-gray-500 hover:text-black text-xl"> &times; </button>
  </div>

  <div className="p-4">
   <label className="block text-gray-700 font-medium"> Sort by Price: </label>
    <select className="w-full p-2 border rounded-md" value={sortingOrder} onChange={handleSortChange} >
  ...
  {distributors.map((distributor) => (
    <p key={distributor} className={`cursor-pointer text-gray-800 hover:text-blue-500 ${ distributorFilter === distributor ? "font-bold text-blue-500" : "" }`}
