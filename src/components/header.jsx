...
const Header = ({ query, setQuery }) => {
  ...
  <div className="hidden xl:flex absolute left-1/2 transform -translate-x-1/2 space-x-8">
   <Link to="/" className="font-bold text-xl text-gray-800 no-underline hover:text-blue-600 transition"> Home </Link>
   <Link to="/about" className="font-bold text-xl text-gray-800 no-underline hover:text-blue-600 transition"> About Us </Link>
   <Link to="/contact" className="font-bold text-xl text-gray-800 no-underline hover:text-blue-600 transition"> Contact</Link>
  </div>
  ...
  <div onClick={handleOpenTabCart} className="cursor-pointer relative">
   ...
   <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full"> {totalQuantity} </span>
  </div>
  ...
  <div className={`absolute xl:hidden top-24 left-0 w-full bg-white flex flex-col items-center gap-6 font-semibold text-lg transition-transform duration-300 transform z-50 ${
